...
json1 := `{"from":"en","to":"zh","trans_result":[{"src":"today","dst":"\u4eca\u5929"},{"src":"tomorrow","dst":"\u660e\u5929"}]}`
	v := gojson.Json(json1).ArrMapValue("trans_result")
	for _, av := range v {
		fmt.Println(av)
	}
*/
func (j *Js) ArrMapValue(arrName string) []map[string]string {
	arrK, arrV := j.Get(arrName).ToArray()
	arrLen := 0
	reMap := make([]map[string]string, 0)
	for _, arrk_v := range arrK {
		if arrK[0] == arrk_v {
			arrLen++
		}
	}
	arr_a_len := len(arrK) / arrLen
	for i := 0; i < arrLen; i++ {
		arr_a_map := make(map[string]string)
		i_c_arr_a_len := i * arr_a_len
		for j := 0; j < arr_a_len; j++ {
			index := i_c_arr_a_len + j
			arr_a_map[arrK[index]] = arrV[index]
		}
		reMap = append(reMap, arr_a_map)
	}
	return reMap
}
